...
import { createContext, ElementType, useContext, useEffect, useState } from "react";
...
export const WithCryptoTickerContext = (
	...
	return function WithCryptoTickerContext(props: any) {
        ...
        const [cryptoTickerSocket, setCryptoTickerSocket] = useState<Socket | null>(null)
        ...
        const initOrGetSocketManaget = () =>{
			...
			_socketManager.on("reconnect", (attempt) => {
				logLevel <= LogLevel.LOG && console.log("Manager Reconnect success: ", attempt);
			});
			_socketManager.on("reconnect_attempt", (attempt) => {
				logLevel <= LogLevel.DEBUG && console.log("Manager Reconnect attempt: ", attempt);
			});
			_socketManager.on("reconnect_error", (err) => {
				logLevel <= LogLevel.DEBUG && console.error("Manager Reconnect err: ", err);
			});
			_socketManager.on("reconnect_failed", () => {
				logLevel <= LogLevel.DEBUG && console.error("Manager Reconnect failed");
			});
        ...
        const initOrGetCryptoTickerSocket =(manager:Manager) =>{
			...
			_socket.on("connect", () => {
				logLevel <= LogLevel.LOG && console.log(`Socket ${_socket.id} connected`);
			});

			_socket.on("disconnect", (reason) => {
				logLevel <= LogLevel.LOG && console.log(`Socket ${_socket.id} disconnect due to ${reason}`);
			});

			_socket.on("connect_error", (error) => {
				logLevel <= LogLevel.DEBUG && console.error("Socket Connect err: ", error);
			});
        ...
        const init = async() =>{
            ...
            const _cryptoTickerSocket = initOrGetCryptoTickerSocket(_socketManager)
